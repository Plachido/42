...
/*   Updated: 2021/04/17 18:20:48 by plpelleg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void ft_check_rgb(t_all *all)
{
	...
	while (i <= 3)
	{
		if(!((all->info->F[i] > 255 || all->info->F[i] < 255) ||
		(all->info->C[i] > 0|| all->info->C[i] < 255)))
...
static void ft_check_resolution(t_all *all)
{
	...
	if ((all->info->R[0] > x) || (all->info->R[1] > y))
	{
		...
		if (all->info->R[1] > y)
			all->info->R[1] = y;
		printf("GIVEN RESOLUTION IS TOO BIG - SETTING TO MAXIMUM\n");
	}
}
